...
The following command moves the first folder to the second folder, not the
other way around. In this case, order matters.
...
The final result is that you have `example/A2/A2`. If there is no folder called
...
An easier way to achieve the same thing is to use a [wildcard
character](https://en.wikipedia.org/wiki/Wildcard_character) `*` to represent
